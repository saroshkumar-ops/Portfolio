...
export interface HyperspeedHandle {
  /** value from 0 (idle) to 1 (full speed-up) — smoothly lerped internally each frame */
...
class App {
  ...
  /** Drives speed-up/FOV directly (0 = idle, 1 = full speed), replacing the
   * original click-and-hold trigger so an external scroll listener can
   * animate this instead. */
  setIntensity(value: number) {
    const clamped = Math.min(1, Math.max(0, value));
    this.speedUpTarget = this.options.speedUp * clamped;
    this.fovTarget = this.options.fov + (this.options.fovSpeedUp - this.options.fov) * clamped;
  }
...
const Hyperspeed = forwardRef<HyperspeedHandle, HyperspeedProps>(function Hyperspeed(
  ...
  useImperativeHandle(ref, () => ({
    setIntensity: (value: number) => {
      appRef.current?.setIntensity(value);
    },
  }));
