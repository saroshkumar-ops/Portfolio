...
function AutoBind<T extends object>(self: T, { include, exclude }: AutoBindOptions = {}): T {
  ...
  const proto = (self as { constructor: { prototype: object } }).constructor.prototype;
  for (const [object, key] of getAllProperties(proto)) {
    if (key === "constructor" || !filter(key)) continue;
    const descriptor = Reflect.getOwnPropertyDescriptor(object, key);
    if (descriptor && typeof descriptor.value === "function") {
      (self as Record<string | symbol, unknown>)[key] = (
        self as Record<string | symbol, (...args: unknown[]) => unknown>
      )[key].bind(self);
    }
  }
  return self;
...
const FlyingPosters = forwardRef<FlyingPostersHandle, FlyingPostersProps>(function FlyingPosters(
  ...
  return (
    <div ref={containerRef} className={`relative z-[2] h-full w-full overflow-hidden ${className}`} {...props}>
